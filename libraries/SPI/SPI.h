...
class SPISettings {
  public:
    SPISettings(uint32_t clock, BitOrder bitOrder, uint8_t dataMode) {
      if(clock >= SPI_SPEED_8MHZ) {
        clk = SPI_SPEED_8MHZ;
      } else if (clock >= SPI_SPEED_4MHZ) {
        clk = SPI_SPEED_4MHZ;
      } else if (clock >= SPI_SPEED_2MHZ) {
        clk = SPI_SPEED_2MHZ;
      } else if (clock >= SPI_SPEED_1MHZ) {
        clk = SPI_SPEED_1MHZ;
      } else if (clock >= SPI_SPEED_500KHZ) {
        clk = SPI_SPEED_500KHZ;
      } else if (clock >= SPI_SPEED_250KHZ) {
        clk = SPI_SPEED_250KHZ;
      } else if (clock >= SPI_SPEED_125KHZ) {
        clk = SPI_SPEED_125KHZ;
      }
  ...
  private:
    spi_speed_e clk;    //specifies the spi bus maximum clock speed
